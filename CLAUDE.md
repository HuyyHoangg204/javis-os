...
Javis là một **AI agentic ĐỔI ĐƯỢC BỘ NÃO**, không phải sản phẩm của một nhà cung cấp nào. Người dùng chọn model chính ở trang Models và đổi lúc nào cũng được; **năng lực của Javis KHÔNG đổi theo model**. Tám bộ não hiện dùng được: Claude Code, ChatGPT (qua Codex), OpenRouter, OpenAI API, Anthropic API, Google Gemini, Groq, Ollama. Hai đường đầu tận dụng chính **gói subscription** người dùng đang trả (không phải mua API riêng); sáu đường sau chỉ cần API key (Ollama ở đây là bản Cloud ollama.com). Nhà cung cấp nào ra sau mà có CLI-agent hoặc API gọi tool đều đấu thêm được.
...
Khác biệt giữa hai nhóm bộ não, nói cho ĐÚNG chứ không nói gọn: hai engine CLI (Claude Code, Codex) có thêm **lệnh máy** (Bash), **WebFetch/WebSearch** (tự mở một URL lạ ra đọc, tự tra web), **Task** (đẻ agent con chạy song song), và nối lại được phiên cũ. Năm engine API không có bốn thứ đó, đọc/ghi brain qua tool vault, mỗi lượt tối đa 8 vòng gọi tool, và khi lượt có gọi tool thì câu trả lời hiện một cục ở cuối chứ không chạy dần từng chữ. Ngoài bốn thứ trên, mọi năng lực còn lại là như nhau.
