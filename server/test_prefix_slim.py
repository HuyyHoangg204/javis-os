"""Test: giảm PHẦN ĐẦU CỐ ĐỊNH mà mọi lượt chat đều phải trả tiền.
    ...
    cd server && ../.venv/Scripts/python.exe test_prefix_slim.py
...
check("tran doc duoc tu bien moi truong", "JAVIS_MEMORY_INDEX_MAX" in
      Path(__file__).resolve().parent.joinpath("main.py").read_text(encoding="utf-8"))

# ---- 3. CLAUDE.md: gỡ mẫu file, GIỮ luật ----
md = Path(__file__).resolve().parent.parent.joinpath("CLAUDE.md").read_text(encoding="utf-8")
...
bld = Path(__file__).resolve().parent.parent.joinpath(
