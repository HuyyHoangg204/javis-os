...
- **Agent** là một "nhân viên AI" có vai trò cố định. Mỗi agent gồm: một cái tên, một mô tả vai trò, một hướng dẫn làm việc chi tiết (system prompt), danh sách kỹ năng (skill) được phép dùng, một **model chạy**, và một **bộ nhớ riêng** tích luỹ theo thời gian. Model chọn được cả **Claude** (Sonnet/Opus/Haiku/Fable - chạy qua Claude Code) lẫn **ChatGPT/Codex** (GPT-5.x - chạy qua Codex CLI, cần đã đăng nhập ChatGPT ở máy/VPS). Cả hai đều đọc/ghi được file trong vault và dùng được MCP. Model của agent được áp THẬT khi workflow chạy.
  - Lưu ý an toàn: khi workflow chạy **nền tự động** (dispatcher Kanban, chế độ giới hạn công cụ file), agent luôn dùng Claude Code để giữ giới hạn công cụ an toàn - kể cả khi bạn chọn model Codex. Model Codex chỉ áp khi bạn bấm **▶ Chạy** trực tiếp ở trang Workflows.
...
- **`MEMORY.md` - bộ nhớ riêng.** Mỗi lần agent chạy, Javis đọc file này và chèn thẳng vào system prompt của agent dưới tiêu đề `# Bộ nhớ của bạn:`. Đây là chỗ để tích luỹ những gì agent cần nhớ lâu dài: quy ước riêng, danh sách khách, những lỗi đã bị nhắc. Không có file thì phần này đơn giản là không xuất hiện, agent vẫn chạy bình thường.
- **`runs/` - nhật ký chạy.** Mỗi bước workflow chạy xong, Javis ghi thêm một mục vào `runs/<YYYY-MM-DD>.md` gồm giờ chạy (giờ Việt Nam), nhiệm vụ đã giao, và kết quả (cắt ở 1500 ký tự). Đây là chỗ để soi lại "hôm qua agent này đã làm gì" mà không cần mở lại bảng theo dõi.
...
Chữ đang gõ dở không bị mất khi bạn gập/mở, đổi thứ tự hay xoá bước khác - Javis lưu tạm nội dung mọi bước trước mỗi lần vẽ lại.
...
Sau mỗi bước xong, Javis ghi kết quả vào nhật ký chạy của agent phụ trách bước đó (`memory/agents/<slug>/runs/`), nên bạn đọc lại được kể cả khi đã đóng bảng.
...
**Brain cũ chưa chuyển cấu trúc** thì hai thư mục đó nằm ở `Javis/agents/` và `Javis/workflows/`. Javis tự dò: có thư mục mới thì dùng thư mục mới, không có thì dùng đường cũ. Nên nếu bạn mở trang Tệp tin mà không thấy `agents/` ở gốc brain, hãy tìm trong `Javis/`.
...
- **Không tìm thấy thư mục `agents/` trong brain.** Brain cũ để ở `Javis/agents/` và `Javis/workflows/`. Mở trang Tệp tin và tìm trong thư mục `Javis`.
...
- [Second Brain: bộ nhớ, Wiki, INGEST](13-second-brain-bo-nho-wiki.md) - phân biệt bộ nhớ riêng của agent với bộ nhớ chung của Javis.
