...
- **Bấm Kết nối hiện "Invalid Scopes: pages_show_list, pages_read_engagement, ..." (hoặc ads_read, business_management)**: app của bạn CHƯA được cấp nhóm quyền đó, không phải sai App ID/Secret và cũng không phải hỏng cấu hình Javis. Trong **giao diện mới**, quyền bị khoá theo trường hợp sử dụng: app tạo bằng trường hợp "Xác thực và yêu cầu dữ liệu từ người dùng" chỉ có `email` và `public_profile`, nên mọi quyền Trang hay quảng cáo đều bị coi là không hợp lệ. Cách sửa:
